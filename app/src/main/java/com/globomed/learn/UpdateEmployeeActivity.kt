...
class UpdateEmployeeActivity: AppCompatActivity() {
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		bundle?.let {
			...
			val employee = empId?.let { it1 -> DataManager.fetchEmployee(databaseHelper, it1) }
			...
			employee?.let {
				...
				sSurgeon.isChecked = (employee.isSurgeon == 1)
	...
	private fun saveEmployee() {
		...
		if (isValid) {
			...
			val updatedEmployee = Employee(empId!!, updateName, updatedDOB, updatedDesignation, updateIsSurgeon)
			DataManager.updateEmployee(databaseHelper, updatedEmployee)

			setResult(Activity.RESULT_OK, Intent())
			Toast.makeText(this, "Employee Updated Sucessfully", Toast.LENGTH_LONG).show()
			...
			R.id.action_delete -> {
				...
				builder.setMessage(R.string.confirm_sure)
					.setPositiveButton(R.string.yes) { dialog, eId ->
						...
						Toast.makeText(this, "$result record deleted", Toast.LENGTH_LONG).show()
						setResult(Activity.RESULT_OK, Intent())
						finish()
					}
